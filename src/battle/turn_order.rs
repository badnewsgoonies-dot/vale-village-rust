...
pub fn calculate_turn_order(units: &[BattleUnit], rng: &mut impl Rng) -> Vec<u32> {
    ...
    entries.sort_by(|a, b| {
        ...
        if spd_cmp != std::cmp::Ordering::Equal {
            return spd_cmp;
        }
...
mod tests {
    ...
    fn test_faster_goes_first() {
        let units = vec![
            make_unit(1, 10, UnitSide::Player),
            make_unit(2, 20, UnitSide::Enemy),
        ];
    ...
    fn test_player_before_enemy_on_tie() {
        let units = vec![
            make_unit(1, 15, UnitSide::Enemy),
            make_unit(2, 15, UnitSide::Player),
        ];
        let mut rng = StdRng::seed_from_u64(42);
        let order = calculate_turn_order(&units, &mut rng);
        assert_eq!(order[0], 2);
    ...
    fn test_ko_units_excluded() {
        let mut units = vec![
            make_unit(1, 20, UnitSide::Player),
            make_unit(2, 30, UnitSide::Enemy),
        ];
        units[1].hp = 0;
        let mut rng = StdRng::seed_from_u64(42);
        let order = calculate_turn_order(&units, &mut rng);
